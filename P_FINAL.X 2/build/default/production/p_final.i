...
void main(void) {
    ...
    ESCRIBE_MENSAJE("MPU OK", 6);
    _delay((unsigned long)((1000)*(64000000UL/4000.0)));
    BORRAR_LCD();

    while (1) {
        MPU6050_DMP_GetData(&ypr, ((void*)0));
        ...
        char buffer1[16];
        sprintf(buffer1, "P: %.1f R: %.1f", pitch, roll);
        POS_CURSOR(1, 1);
        ESCRIBE_MENSAJE(buffer1, 16);

        POS_CURSOR(2, 0);

        if (roll > 35) {
            ESCRIBE_MENSAJE("No   ", 10);
        } else if (roll < -10) {
            ESCRIBE_MENSAJE("Si ", 10);
        } else if (pitch > 73) {
            ESCRIBE_MENSAJE("Hola    ", 10);
        } else if (pitch < 40) {
            ESCRIBE_MENSAJE("Gracias     ", 10);
        } else {
            ESCRIBE_MENSAJE("Esperando movimiento", 20);
        }
