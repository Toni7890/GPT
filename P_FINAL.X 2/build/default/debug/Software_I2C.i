...
sw_i2c_error_t SW_I2C_Start(void) {
    ...
    (TRISBbits.TRISB2 = 1);
    (TRISBbits.TRISB1 = 1);
    do { switch(current_i2c_speed) { case SW_I2C_SPEED_FAST: _delay((unsigned long)((5)*(64000000UL/4000000.0))); break; case SW_I2C_SPEED_NORMAL: _delay((unsigned long)((8)*(64000000UL/4000000.0))); break; case SW_I2C_SPEED_ROBUST: _delay((unsigned long)((12)*(64000000UL/4000000.0))); break; default: _delay((unsigned long)((8)*(64000000UL/4000000.0))); break; } } while(0);
...
sw_i2c_error_t SW_I2C_Restart(void) {
    ...
    SW_I2C_Stop();
    _delay((unsigned long)((8)*(64000000UL/4000000.0)));
    return SW_I2C_Start();
}
...
uint8_t SW_I2C_IsBusFree(void) {
    ...
    return (PORTBbits.RB1 && PORTBbits.RB2);
}
...
sw_i2c_error_t SW_I2C_ReadBlock(uint8_t device_addr, uint8_t reg_addr, uint8_t *data, uint8_t length) {
    ...
    for (i = 0; i < length; i++) {
        if (i == (length - 1)) {
            data[i] = SW_I2C_ReadByte(0);
        } else {
            data[i] = SW_I2C_ReadByte(1);
        }
    }
...
uint8_t SW_I2C_ScanBus(uint8_t *found_devices, uint8_t max_devices) {
    ...
    if (found_devices == ((void*)0) || max_devices == 0) {
        return 0;
    }
    ...
    for (addr = 0x08; addr <= 0x77 && devices_found < max_devices; addr++) {
        ...
        if (result == SW_I2C_SUCCESS) {
            ...
            result = SW_I2C_WriteByte((uint8_t)((addr << 1) | 0));
            if (result == SW_I2C_SUCCESS) {
...
uint8_t SW_I2C_GetPinStatus(void) {
    ...
    if (PORTBbits.RB1) status |= 0x01;
    if (PORTBbits.RB2) status |= 0x02;
